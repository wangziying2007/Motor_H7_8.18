...
void MX_FDCAN2_Init(void)
{
  ...
  /* 配置标准帧滤波器：接收电机回传帧 ID 0x200 ~ 0x208（M3508 反馈 0x201~0x208），入 RxFIFO0 */
  FDCAN_FilterTypeDef sFilterConfig = {0};
  sFilterConfig.IdType       = FDCAN_STANDARD_ID;
  sFilterConfig.FilterIndex  = 0;
  sFilterConfig.FilterType   = FDCAN_FILTER_RANGE;
  sFilterConfig.FilterConfig = FDCAN_FILTER_TO_RXFIFO0;
  sFilterConfig.FilterID1    = 0x200U;   /* range 下界 */
  sFilterConfig.FilterID2    = 0x208U;   /* range 上界 */
  if (HAL_FDCAN_ConfigFilter(&hfdcan2, &sFilterConfig) != HAL_OK)
  {
    Error_Handler();
  }

  /* 全局过滤器：未匹配的标准帧也放进 FIFO0（保险），拒绝所有远程帧 */
  if (HAL_FDCAN_ConfigGlobalFilter(&hfdcan2, FDCAN_ACCEPT_IN_RX_FIFO0,
                                   FDCAN_REJECT, FDCAN_REJECT_REMOTE, FDCAN_REJECT_REMOTE) != HAL_OK)
  {
    Error_Handler();
  }
